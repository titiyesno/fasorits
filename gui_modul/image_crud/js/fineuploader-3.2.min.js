...
qq.isFileOrInput = function(a) {
    if (window.File && a instanceof File)
        return!0;
    if (window.HTMLInputElement) {
        if (a instanceof HTMLInputElement && a.type && "file" === a.type.toLowerCase())
            return!0
    } else if (a.tagName && "input" === a.tagName.toLowerCase() && a.type && "file" === a.type.toLowerCase())
        return!0;
    return!1
};
...
qq.each = function(a, b) {
    ...
    if (a)
        for (c in a)
            if (Object.prototype.hasOwnProperty.call(a, c) && (d = b(c, a[c]), !1 === d))
                break
};
qq.obj2url = function(a, b, c) {
    ...
    if (!c && b)
        f = /\?/.test(b) ? /\?$/.test(b) ? "" : "&" : "?", e.push(b), e.push(qq.obj2url(a));
...
qq.UploadButton = function(a) {
    this._options = {element: null, multiple: !1, acceptFiles: null, name: "file", onChange: function() {
...
qq.UploadButton.prototype = {getInput: function() {
        ...
        qq(a).css({position: "absolute",
            right: 0, top: 0, fontFamily: "Arial", fontSize: "118px", margin: 0, padding: 0, cursor: "pointer", opacity: 0});
...
qq.FineUploaderBasic = function(a) {
    this._options = {debug: !1, button: null, multiple: !0, maxConnections: 3, disableCancelForFormUploads: !1, autoUpload: !0, request: {endpoint: "/server/upload", params: {}, paramsInBody: !1, customHeaders: {}, forceMultipart: !0, inputName: "qqfile", uuidName: "qquuid", totalFileSizeName: "qqtotalfilesize"}, validation: {allowedExtensions: [], sizeLimit: 0, minSizeLimit: 0, stopOnFirstInvalidFile: !0}, callbacks: {onSubmit: function() {
            }, onComplete: function() {
            }, onCancel: function() {
            }, onUpload: function() {
            },
            ...
            }}, messages: {typeError: "{file} has an invalid extension. Valid extension(s): {extensions}.", sizeError: "{file} is too large, maximum file size is {sizeLimit}.", minSizeError: "{file} is too small, minimum file size is {minSizeLimit}.", emptyError: "{file} is empty, please select files again without it.", noFilesError: "No files to upload.",
            ...
            id: null, cookiesExpireIn: 7, paramNames: {resuming: "qqresume"}}, formatFileName: function(a) {
            33 < a.length && (a = a.slice(0, 19) + "..." + a.slice(-14));
            return a
        }, text: {sizeSymbols: "kB MB GB TB PB EB".split(" ")}};
...
qq.FineUploaderBasic.prototype = {log: function(a, b) {
        ...
        for (var a; this._storedFileIds.length; )
            a = this._storedFileIds.shift(),
    ...
    addFiles: function(a) {
        var b = [], c, d;
        if (a) {
            if (!window.FileList || !(a instanceof FileList))
                a = [].concat(a);
            for (c = 0; c < a.length; c += 1)
                d = a[c], qq.isFileOrInput(d) ? b.push(d) : this.log(d + " is not a File or INPUT element!  Ignoring!", "warn");
    ...
    _createUploadButton: function(a) {
        var b = this, c = new qq.UploadButton({element: a, multiple: this._options.multiple && qq.isXhrUploadSupported(), acceptFiles: this._options.validation.acceptFiles, onChange: function(a) {
                b._onInputChange(a)
    ...
    _onSubmit: function(a) {
        ...
        if (this._handler.isValid(a)) {
            ...
            if (!1 === this._options.callbacks.onManualRetry(a, b))
                return!1;
        ...
        if (!c.success)
            if (d && 200 !== d.status && !c.error)
                ...
                this._options.callbacks.onError(a, b, c.error ? c.error : "Upload failure reason unknown")
    }, _uploadFileList: function(a) {
        var b;
        if (!1 !== this._options.callbacks.onValidateBatch(this._getValidationDescriptors(a)))
            if (0 < a.length)
                for (b = 0; b < a.length; b++)
                    if (this._validateFile(a[b]))
                        this._uploadFile(a[b]);
                    else {
        ...
        var a = this._handler.add(a), b = this._handler.getName(a);
        ...
        b = a.name;
        c = a.size;
        if (!1 === this._options.callbacks.onValidate(a))
            return!1;
        if (this._isAllowedExtension(b)) {
            if (0 === c)
                return this._error("emptyError",
                        b), !1;
            if (c && this._options.validation.sizeLimit && c > this._options.validation.sizeLimit)
                return this._error("sizeError", b), !1;
            if (c && c < this._options.validation.minSizeLimit)
                return this._error("minSizeError", b), !1
        } else
            return this._error("typeError", b), !1;
        return!0
    }, _error: function(a, b) {
        var c = this._options.messages[a], d = this._options.validation.allowedExtensions.join(", ").toLowerCase(), e = this._options.formatFileName(b), c = c.replace("{file}", e), c = c.replace("{extensions}", d), d = this._formatSize(this._options.validation.sizeLimit),
                c = c.replace("{sizeLimit}", d), d = this._formatSize(this._options.validation.minSizeLimit), c = c.replace("{minSizeLimit}", d);
    ...
    _wrapCallbacks: function() {
        ...
        var a = {}, b = this;
        return{setEndpoint: function(b, d) {
                a[d] = b
...
qq.DragAndDrop = function(a) {
    ...
    function d(a) {
        h = new qq.UploadDropZone({element: a, onEnter: function(b) {
            ...
            onDrop: function(d) {
                ...
                qq(a).removeClass(g.classes.dropActive);
                var d = d.dataTransfer, e, f;
                g.callbacks.dropProcessing(!0);
                h.dropDisabled(!0);
                if (1 < d.files.length && !g.multiple)
                    g.callbacks.dropProcessing(!1), g.callbacks.error("tooManyFilesError", ""), h.dropDisabled(!1);
                else if (l = [], n = p = 0, qq.isFolderDropSupported(d)) {
                    e = d.items;
                    for (d = 0; d < e.length; d += 1)
                        if (f = e[d].webkitGetAsEntry())
                            f.isFile ? (l.push(e[d].getAsFile()), d === e.length - 1 && b()) : c(f)
                } else
                    g.callbacks.dropProcessing(!1, d.files),
                            h.dropDisabled(!1)
            }});
    ...
    var g, h, k, l = [], p = 0, n = 0, q = new qq.DisposeSupport;
    g = {dropArea: null, extraDropzones: [], hideDropzones: !0, multiple: !0, classes: {dropActive: null}, callbacks: {dropProcessing: function() {
            }, error: function() {
            },
            log: function() {
            }}};
...
qq.FineUploader = function(a) {
    ...
    qq.extend(this._options, {element: null, listElement: null, dragAndDrop: {extraDropzones: [], hideDropzones: !0, disableDefaultDropzone: !1}, text: {uploadButton: "Upload a file", cancelButton: "Cancel", retryButton: "Retry", failUpload: "Upload failed", dragZone: "Drop files here to upload", dropProcessing: "Processing dropped files...", formatProgress: "{percent}% of {total_size}", waitingForResponse: "Processing..."}, template: '<div class="qq-uploader">' +
        ...
        classes: {button: "qq-upload-button", drop: "qq-upload-drop-area", dropActive: "qq-upload-drop-area-active", dropDisabled: "qq-upload-drop-area-disabled", list: "qq-upload-list", progressBar: "qq-progress-bar", file: "qq-upload-file", spinner: "qq-upload-spinner", finished: "qq-upload-finished", retrying: "qq-upload-retrying", retryable: "qq-upload-retryable", size: "qq-upload-size", cancel: "qq-upload-cancel", retry: "qq-upload-retry", statusText: "qq-upload-status-text", success: "qq-upload-success", fail: "qq-upload-fail", successIcon: null,
            failIcon: null, dropProcessing: "qq-drop-processing", dropProcessingSpinner: "qq-drop-processing-spinner"}, failedUploadTextDisplay: {mode: "default", maxChars: 50, responseProperty: "error", enableTooltip: !0}, messages: {tooManyFilesError: "You may only drop one file"}, retry: {showAutoRetryNote: !0, autoRetryNote: "Retrying {retryNum}/{maxAuto}...", showButton: !1}, showMessage: function(a) {
            setTimeout(function() {
                alert(a)
            }, 0)
        }}, !0);
...
qq.extend(qq.FineUploader.prototype, {clearStoredFiles: function() {
        ...
        var a = this, b = this._find(this._element, "dropProcessing"),
                c, d;
        d = function(a) {
            a.preventDefault()
        };
        this._options.dragAndDrop.disableDefaultDropzone || (c = this._find(this._options.element, "drop"));
        c = new qq.DragAndDrop({dropArea: c, extraDropzones: this._options.dragAndDrop.extraDropzones, hideDropzones: this._options.dragAndDrop.hideDropzones, multiple: this._options.multiple, classes: {dropActive: this._options.classes.dropActive}, callbacks: {dropProcessing: function(c, f) {
                    var g = a._button.getInput();
                    c ? (qq(b).css({display: "block"}), qq(g).attach("click", d)) : (qq(b).hide(), qq(g).detach("click",
                            d));
                    f && a.addFiles(f)
                }, error: function(b, c) {
                    a._error(b, c)
        ...
        c === d ? (g = this._find(e, "cancel"), qq(g).hide(), qq(f).hide(), qq(this._find(e, "statusText")).setText(this._options.text.waitingForResponse), g = this._formatSize(d)) : (g = this._formatProgress(c, d), qq(f).css({display: "block"}));
        qq(f).css({width: h + "%"});
        e = this._find(e, "size");
        ...
        if (!this._options.disableCancelForFormUploads || qq.isXhrUploadSupported())
            this._find(a, "cancel").style.display = "inline"
    }, _error: function(a, b) {
...
qq.UploadHandlerForm = function(a, b, c) {
    ...
    function e(a) {
        var b;
        try {
            var c = a.contentDocument || a.contentWindow.document, d = c.body.innerHTML;
            ...
            n("Error when attempting to parse form upload response (" + e + ")", "error"), b = {success: !1}
        }
        return b
    }
    function f(a) {
    ...
    function g(a, b) {
        var c = h.paramsStore.getParams(a), d = qq.toElement('<form method="' + (h.demoMode ? "GET" : "POST") + '" enctype="multipart/form-data"></form>'),
                e = h.endpointStore.getEndpoint(a), f = e;
    ...
    return q = {add: function(a) {
            ...
            if (a = document.getElementById(a))
                a.setAttribute("src", "java" + String.fromCharCode(115) + "cript:false;"), qq(a).remove()
        }, upload: function(a) {
            var c = k[a], l = q.getName(a), s = f(a), i = g(a, s);
            if (!c)
            ...
            d(s, function() {
                ...
                if (c.success || !h.onAutoRetry(a, l, c))
                    h.onComplete(a, l, c), b(a)
            });
...
qq.UploadHandlerXhr = function(a, b, c) {
    function d(a, b, c) {
        var d = m.getSize(a), a = m.getName(a);
    ...
    function e(a, b) {
        var c = i.chunking.partSize, d = m.getSize(a), e = j[a].file, g = c * b, c = g + c >= d ? d : g + c, d = f(a), h;
        e.slice ? h = e.slice(g, c) : e.mozSlice ? h = e.mozSlice(g, c) : e.webkitSlice &&
                (h = e.webkitSlice(g, c));
        return{part: b, start: g, end: c, count: d, blob: h}
    }
    function f(a) {
        a = m.getSize(a);
    ...
    function h(a, b, c, d) {
        var e = new FormData, f = i.demoMode ? "GET" : "POST", g = i.endpointStore.getEndpoint(d), h = g, k = m.getName(d), l = m.getSize(d);
    ...
    function l(a, b, c) {
        var d = m.getName(a), e = m.getSize(a);
    ...
    function p(a) {
        var b = e(a, j[a].remainingChunkIdxs[0]), c = g(a), f = m.getSize(a), A = m.getName(a), t;
        void 0 === j[a].loaded && (j[a].loaded = 0);
        B(a, b);
        c.onreadystatechange = q(a, c);
        c.upload.onprogress = function(b) {
            if (b.lengthComputable && j[a].loaded < f)
                i.onProgress(a, A, b.loaded + j[a].loaded, f)
        };
                ...
                if (j[a]) {
                    ...
                    if (200 !== b.status || !c.success || c.reset)
                        if (c.reset && (r("Server has ordered chunking effort to be restarted on next attempt for file ID " +
                                a, "error"), w && y(a), j[a].remainingChunkIdxs = [], delete j[a].loaded), j[a].attemptingResume && c.reset)
                            j[a].attemptingResume = !1, r("Server has declared that it cannot handle resume for file ID " + a + " - starting from the first chunk", "error"), m.upload(a, !0);
                        else {
                            var f = m.getName(a);
                            i.onAutoRetry(a, f, c, b) || l(a, c, b)
                        }
                    else
                        x ? (f = j[a].remainingChunkIdxs.shift(), f = e(a, f), j[a].attemptingResume = !1, j[a].loaded += f.end - f.start, 0 < j[a].remainingChunkIdxs.length ? p(a) : (y(a), l(a, c, b))) : l(a, c, b)
                }
            }
        }
    }
    ...
    function z(a) {
        var b = m.getName(a), a = m.getSize(a), c = i.chunking.partSize, b = "qqfilechunk" + o + encodeURIComponent(b) + o + a + o + c;
        void 0 !== u && (b += o + u);
        return b
    }
    function s(a) {
        var b = j[a].file, c = m.getName(a), d, e;
        j[a].loaded = 0;
        d = g(a);
    ...
    var i = a, C = b, r = c, j = [], o = "|", x = i.chunking.enabled && qq.isFileChunkingSupported(), w = i.resume.enabled && x && qq.areCookiesEnabled(), u;
    u = null !== i.resume.id && void 0 !== i.resume.id && !qq.isFunction(i.resume.id) && !qq.isObject(i.resume.id) ? i.resume.id : void 0;
    var v = i.forceMultipart || i.paramsInBody, m;
            ...
            if (x) {
                ...
                if (!j[a].remainingChunkIdxs || 0 === j[a].remainingChunkIdxs.length) {
                    ...
                    if (w && !b) {
                        h = qq.getCookie(z(a));
                        var k;
                        h ? (k = h.indexOf(o), g = h.substr(0, k), h = parseInt(h.substr(k + 1, h.length - k), 10), g = {uuid: g, part: h}) : g = void 0;
                    ...
                    for (d = f(a) - 1; d >= c; d -= 1)
                        j[a].remainingChunkIdxs.unshift(d)
                }
                p(a)
            } else
                s(a)
