...
class s extends Super_Controller {
    ...
    public function index($index=5) {
        $data['listberita'] = $this->artikel_model->getnewsberita($index,$this->uri->segment(4));
        $menu = "hf/menu/menu_pengelola.php";
        $subnav = "subnav.php";
        $footer = "hf/footer/footer.php";
        $config['total_rows'] = $this->artikel_model->totalArtikel();
  
        $config['base_url'] = base_url()."index.php/news/s/index/";
        $config['per_page'] = 5;
        ...
        $this->template->build("s_lihatberita.php", $data);
    }
    ...
    public function tambah() {
        ...
        $data['jenis'] = $this->artikel_model->getjenisberita(2);
        $this->template->set_layout('back_end');
        $this->template->title("Home Admin");
        $this->template->set_partial("menu", $menu);
        $this->template->set_partial("footer", $footer);
    ...
    public function getberita($idberita = "") {
        ...
        $this->template->set_partial("menu", $menu);
        $this->template->set_partial("footer", $footer);
        $this->template->build("s_viewsingle.php", $data);
    }
    ...
    public function simpanoraddedit() {
        ...
        if (!isset($id) || $id == 0) {
            ...
            $data['admin_idadmin'] = $this->session->userdata['logged_in']['id'];
            $idberita = $this->artikel_model->create_berita($data);
            redirect("news/s/getberita/" . $idberita);
        } else {
            $id = $this->input->post('id');
            $data['judul'] = $this->input->post('judul');
            $data['vr'] = $this->input->post('vr');
